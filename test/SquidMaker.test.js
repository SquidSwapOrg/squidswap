...
const SquidMaker = artifacts.require('SquidMaker');
const MockERC20 = artifacts.require('MockERC20');
const UniswapV2Pair = artifacts.require('UniswapV2Pair');
const UniswapV2Factory = artifacts.require('UniswapV2Factory');

contract('SquidMaker', ([alice, bar, minter]) => {
    beforeEach(async () => {
        this.factory = await UniswapV2Factory.new(alice, { from: alice });
        this.squid = await SquidToken.new({ from: alice });
        await this.squid.mint(minter, '100000000', { from: alice });
        ...
        this.maker = await SquidMaker.new(this.factory.address, bar, this.squid.address, this.weth.address);
        this.squidWETH = await UniswapV2Pair.at((await this.factory.createPair(this.weth.address, this.squid.address)).logs[0].args.pair);
    ...
    it('should make SQUIDs successfully', async () => {
        await this.factory.setFeeTo(this.maker.address, { from: alice });
        await this.weth.transfer(this.squidWETH.address, '10000000', { from: minter });
        await this.squid.transfer(this.squidWETH.address, '10000000', { from: minter });
        await this.squidWETH.mint(minter);
        ...
        // After calling convert, bar should have squid value at ~1/6 of revenue
        await this.maker.convert(this.token1.address, this.token2.address);
        assert.equal((await this.squid.balanceOf(bar)).valueOf(), '32965');
        assert.equal((await this.token1Token2.balanceOf(this.maker.address)).valueOf(), '0');
        // Should also work for SQUID-ETH pair
        await this.squid.transfer(this.squidWETH.address, '100000', { from: minter });
        await this.weth.transfer(this.squidWETH.address, '100000', { from: minter });
        await this.squidWETH.sync();
        await this.squid.transfer(this.squidWETH.address, '10000000', { from: minter });
        await this.weth.transfer(this.squidWETH.address, '10000000', { from: minter });
        await this.squidWETH.mint(minter);
        assert.equal((await this.squidWETH.balanceOf(this.maker.address)).valueOf(), '16537');
        await this.maker.convert(this.squid.address, this.weth.address);
        assert.equal((await this.squid.balanceOf(bar)).valueOf(), '66249');
        assert.equal((await this.squidWETH.balanceOf(this.maker.address)).valueOf(), '0');
    });
});